...
**EWP-009 assessment (2026-07-21):** performed as required by this gate. EWP-009's locked scope (`docs/phase-7-charter.md`) provisions the Supabase staging project only -- no Next.js deployment exists anywhere as part of it. This item's actual risk (bundled `next` CVEs) is therefore **not triggered by EWP-009 as executed**: no Next.js code runs anywhere reachable yet. A separate, non-Next.js reachability fact remains true regardless -- the Supabase project's own PostgREST/Auth API is internet-reachable by design the moment a hosted project exists (inherent to Supabase Cloud, not an EWP-009 decision) -- but that surface's protection is RLS-as-final-authority and `service_role` isolation (both independently verified live during EWP-009), not this item's concern. This gate re-activates the moment any future EWP (most plausibly EWP-011, Signup/Login UI) deploys the Next.js application somewhere reachable -- flagged here so it is not silently forgotten when that EWP is proposed.

**EWP-011 assessment (2026-07-24):** this gate has now re-activated, exactly as anticipated by the note above. EWP-011 deployed the Next.js application (the same `next@14.2.35` this item tracks) to a Vercel Preview environment, and that Preview is internet-reachable. No deployment-access-control mitigation (e.g. Vercel Preview password/SSO protection, IP allowlisting) has been documented or applied in this workflow. This is recorded here as a factual state, not as an EWP-011 failure or a security incident -- EWP-011's own charter scope was the signup/login UI and its exit criterion, not a TD-007 resolution or mitigation decision. This item's existing **binding resolution gate before unrestricted Public Production LIVE** (above) is unchanged by this note; it continues to apply exactly as already written. Any decision to apply an access-control mitigation to the current Preview, or to proceed toward Public Production LIVE, remains a separate, explicitly-authorized step, not implied by this assessment.

---

## TD-008 — Undocumented implicit `PUBLIC` EXECUTE grant on three `identity` routines — RESOLVED

**Logged:** 2026-07-21, during EWP-009 (Phase 7) staging live-verification. **Resolved:** 2026-07-21, via `010_secure_identity_routine_execute.sql`, per Founder/Chief-Architect-approved remediation.

**Where:**
`identity.current_user_id()`, `identity.is_admin()`, `identity.set_updated_at()` (`001_create_identity_schema.sql`).

**What:**
Migration 001 never explicitly granted or revoked `EXECUTE` on these three functions, leaving Postgres's implicit `PUBLIC`-execute default (applied automatically at function creation) still in effect. Confirmed live via `has_function_privilege()`/`proacl` inspection against the real staging project (not assumed): all three showed `proacl = NULL` and `anon_execute = true` -- unlike `api`'s 15 routines (`008_create_api_schema.sql`), which all carry an explicit ACL deliberately excluding `anon`.

**Why it was debt:**
Exactly one control -- `anon` lacking `USAGE` on schema `identity` -- was the only thing preventing this already-present, undocumented grant from being exercised. A single future change granting `anon` that `USAGE` for any unrelated reason would have activated anonymous execution of three `SECURITY DEFINER` functions silently, with no additional step. A single point of failure, not an active exploit.
