...
Implements the charter's EWP-011 scope: minimal registration/login forms calling Supabase Auth directly, plus an advisory, client-side username-availability check. **Database layer and application code are complete and independently verified on staging; the Next.js application itself is not deployed anywhere, so EWP-011's own exit criterion ("a real person can sign up and log in through the deployed staging UI") is not yet met** — that requires its own separate TD-007 resolve-or-mitigate decision and hosting authorization before proceeding.
...
**Two real issues found and fixed during validation — disclosed here, not silently corrected:**
...
**Remaining for full EWP-011 closure:** Next.js application hosting for staging (Vercel or otherwise) is not yet provisioned — no cost or external resource has been committed. TD-007's binding resolve-or-mitigate gate (`docs/technical-debt.md`) must be explicitly decided before that deployment proceeds, as its own separately-authorized step.
